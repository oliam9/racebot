"""
FIA Formula 2 Connector.
Scrapes https://www.fiaformula2.com/Calendar for schedule data.
"""
...
class F2Connector(Connector):
    """
    Connector for FIA Formula 2 Championship.
    ...
    def fetch_season(self, series_id: str, season: int) -> RawSeriesPayload:
        """Fetch F2 season schedule."""
        if series_id != "f2":
            raise ValueError(f"F2 connector does not support series: {series_id}")
        
        # FIA F2/F3 sites often have API endpoints
        # api_endpoints = [
        #     f"{self.BASE_URL}/api/calendar/{season}",
        #     f"{self.BASE_URL}/api/events?season={season}",
        #     f"https://api.fiaformula2.com/v1/event-tracker?seasonYear={season}",
        # ]
        ...
        try:
            ...
            logger.error(f"Failed to fetch F2 calendar: {e}")
            raise
    
    def extract(self, payload: RawSeriesPayload) -> List[Event]:
        """Parse F2 season data into Event objects."""
        series_id = payload.metadata.get("series_id", "f2")
        ...
        try:
            ...
            for item in events_list:
                try:
                    ...
                    logger.warning(f"Failed to parse F2 event: {e}")
                    
        except Exception as e:
            logger.error(f"Failed to parse F2 JSON: {e}")
    ...
    def _parse_event_json(self, item: Dict[str, Any], series_id: str, season: int, source_url: str) -> Optional[Event]:
        ...
        # Create sessions (F2 typically has practice, qualifying, sprint, feature race)
    ...
    def _parse_html(self, payload: RawSeriesPayload, series_id: str, season: int) -> List[Event]:
        ...
        try:
                    ...
                    if races:
                        ...
                        for item in races:
                            try:
                                ...
                                logger.warning(f"Failed to parse F2 Next.js event: {e}")
                        return events
                except Exception as e:
            ...
            logger.warning("F2 HTML parsing requires browser rendering - consider using generic connector")
            
        except Exception as e:
            logger.error(f"Failed to parse F2 HTML: {e}")
    ...
    def _parse_nextjs_event(self, item: Dict[str, Any], series_id: str, season: int, source_url: str) -> Optional[Event]:
        ...
        for sess_item in item.get("Sessions", []):
            try:
                ...
                elif "RACE" in s_code or "RESULT" in s_code: # F2 typically uses RESULT for races in this feed
                ...
                # Timestamps
                # "os" times in JSON are usually UTC or local with offset? 
                # Example: "2026-03-06T08:45:00+11:00" -> ISO format with offset
                start_time = None
                end_time = None
                ...
                # available statuses: SCHEDULED, UPDATED, CANCELLED, TBD
                if sess_item.get("Unconfirmed"):
                     # mapping unconfirmed to TBD or keeping SCHEDULED?
                     # Since we have times, SCHEDULED is more appropriate.
                     pass
    ...
    def _create_default_sessions(self, event_id: str, season: int) -> List[Session]:
        """Create default sessions for F2 (typically support race format)."""
        sessions = []
        
        # F2 format: Practice, Qualifying, Sprint Race, Feature Race
        session_info = [
