...
class Moto2Connector(Connector):
    """
    Connector for Moto2 using the official API (api.pulselive.motogp.com).
    ...
    def __init__(self):
        super().__init__()
        self._moto2_category_id = None  # Cache Moto2 category ID
        
    @property
    def id(self) -> str:
        return "moto2_official"

    @property
    def name(self) -> str:
        return "Moto2 Official API"
    ...
    def _get_season_id(self, year: int) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"Moto2 season {year} not found in API")
            return None
            
        except Exception as e:
            logger.error(f"Failed to fetch Moto2 seasons: {e}")
            return None
    
    def _get_moto2_category_id(self, season_uuid: str) -> Optional[str]:
        """Fetch the Moto2 category UUID (cached)."""
        if self._moto2_category_id:
            return self._moto2_category_id
        ...
        try:
            ...
            for cat in categories:
                # Moto2 has legacy_id=2 or name contains "Moto2"
                if cat.get("legacy_id") == 2 or "Moto2" in cat.get("name", ""):
                    self._moto2_category_id = cat.get("id")
                    return self._moto2_category_id
            
            logger.warning("Moto2 category not found")
            return None
            
        except Exception as e:
            logger.error(f"Failed to fetch Moto2 categories: {e}")
            return None
    ...
    def fetch_season(self, series_id: str, season: int) -> RawSeriesPayload:
        ...
        if series_id != "moto2":
            raise ValueError(f"Moto2 connector does not support series: {series_id}")
        ...
        try:
            ...
            logger.error(f"Failed to fetch Moto2 calendar: {e}")
            raise
    ...
    def extract(self, raw: RawSeriesPayload) -> List[Event]:
        ...
        try:
            ...
            logger.error("Failed to parse Moto2 API response")
            return []
    ...
    def _parse_event(self, data: Dict[str, Any], raw: RawSeriesPayload) -> Optional[Event]:
        ...
        if event_id and season_uuid:
            category_id = self._get_moto2_category_id(season_uuid)
            if category_id:
        ...
        source = Source(
            url=raw.url,
            provider_name="Moto2 Official API",
        ...
        return Event(
            event_id=data.get("id"),
            series_id=raw.metadata.get("series_id", "moto2"),
            name=name,
