"""
FIM Supersport World Championship Connector.
Uses the same worldsbk.com platform as WorldSBK.
"""
...
class FIMSupersportConnector(Connector):
    """Connector for FIM Supersport World Championship (same site as WorldSBK)."""

    SCHEDULE_URL = "https://www.worldsbk.com/en/calendar"  # Shared with SBK

    @property
    def id(self) -> str:
        return "fim_supersport_official"

    @property
    def name(self) -> str:
        return "FIM Supersport Official"

    def supported_series(self) -> List[SeriesDescriptor]:
        return [SeriesDescriptor(series_id="fim_supersport",
    ...
    def fetch_season(self, series_id: str, season: int) -> RawSeriesPayload:
        if series_id != "fim_supersport":
            raise ValueError(f"FIM Supersport connector does not support: {series_id}")
        html, method = "", "http"
        if self.playwright_enabled:
            try:
                ...
                logger.warning("Playwright failed for FIM Supersport: %s", e)
        if not html:
    ...
    def extract(self, raw: RawSeriesPayload) -> List[Event]:
        ...
        if not containers:
            containers = soup.find_all("a", href=re.compile(r"/round/|/event/|/race/"))
        ...
        for idx, c in enumerate(containers, 1):
            try:
                ...
                name = lines[0][:60] if lines else f"WorldSSP Rd {idx}"
                tz, _ = infer_timezone_from_location(city=name, country="")
                events.append(Event(
                    event_id=f"wssp_{season}_r{idx}", series_id="fim_supersport", name=name,
                ...
                logger.debug("FIM Supersport %d: %s", idx, e)
        logger.info("FIM Supersport: %d events for %d", len(events), season)
        return events
